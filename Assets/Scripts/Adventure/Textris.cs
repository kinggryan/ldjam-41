...
public partial class @Textris: Cradle.StoryFormats.Harlowe.HarloweStory
{
	...
	IStoryThread passage14_Main()
	{
		...
		Vars.location["TITLE"]  = false;
		Vars.guard  = false;
		yield return text("You walk into the Security Room. Big console of security monitors, currently malfunctioning. Guard lying next to the console. You LOOK at large calendar on the wall. Warehouse to the WEST.");
		yield return lineBreak();
	...
	IStoryThread passage18_Main()
	{
		...
		if(Vars.inv["FOB"] == true) {
			if(Vars.location["SERVER"] == true && Vars.open_cab == false) {
				Vars.open_cab  = true;
				yield return text(" You tap the key FOB onto the padlock. Cabinet swings open. Inside you find cables, dongles, a data stick labeled CURE. ");
				using (Group("hook", "uc")) {
